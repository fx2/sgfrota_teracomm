...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb back-transparente">
            ...
            <li class="breadcrumb-item">Manutenção/Despesa</li>
        </ol>
    </nav>

    <div class="card">
        <div class="card-header h3">Manutenção/Despesa</div>
        <div class="card-body">
            @can('checksetor', MANUTENCAODESPESAS_ADICIONAR)
                <a href="{{ url('/manutencao/create') }}" class="btn btn-success btn-sm" title="Add New Manutenção/Despesa">
                    <i class="fa fa-plus" aria-hidden="true"></i> Adicionar
                </a>
            @endcan

            <form method="GET" action="{{ url('/manutencao/custom/listagem ') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                            ...
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                                <div class="row">
                                    ...
                                    @include('parts/tipo-manutencao-despesa-select')
...
{{--                                <td>{{ $loop->iteration }}</td>--}}
                                ...
                                @endforeach
                                <td>
                                    {{-- <a href="{{ url('/manutencao/' . $item->id) }}" title="Visualizar Manutenção/Despesa"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button></a> --}}
                                    @can('checksetor', MANUTENCAODESPESAS_EDITAR)
                                        <a href="{{ url('/manutencao/' . $item->id . '/edit') }}" title="Editar Manutenção/Despesa"><button class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></button></a>
                                    @endcan

                                    @can('checksetor', MANUTENCAODESPESAS_DELETAR)
                                        <button type="submit" data-id="{{ $item->id }}" data-route="/manutencao" class="btnDeletar btn btn-danger btn-sm" title="Deletar Manutenção/Despesa"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                    @endcan
                                </td>
                            </tr>
                        @empty
                            <p class="p-1 hide-thead" onload="hideThead()">Nenhum Manutenção/Despesa encontrado</p>
                        @endforelse
                    </tbody>
                </table>
