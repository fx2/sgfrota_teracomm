...
@section('content')
    ...
    <div class="card">
        <div class="card-header h3">Agenda</div>
        <div class="card-body">
            @can('checksetor', _ADICIONAR)
                <a href="{{ url('/agenda/create') }}" class="btn btn-success btn-sm" title="Add New Agenda">
                        ...
                        @forelse($results as $item)
                            <tr>
                                ...
                                @endcan
                                    ...
                                    @endforeach
                                <td>
                                    {{-- <a href="{{ url('/agenda/' . $item->id) }}" title="Visualizar Agenda"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button></a> --}}
                                    @can('checksetor', _EDITAR)
                                        <a href="{{ url('/agenda/' . $item->id . '/edit') }}" title="Editar Agenda"><button class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></button></a>
                                    @endcan

                                    @can('checksetor', _DELETAR)
                                        <button type="submit" data-id="{{ $item->id }}" data-route="/agenda" class="btnDeletar btn btn-danger btn-sm" title="Deletar Agenda"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                    @endcan
                                </td>
                            </tr>
                        @empty
                            <p class="p-1 hide-thead" onload="hideThead()">Nenhum Agenda encontrado</p>
                        @endforelse
                    </tbody>
                </table>
